...
def index(request):
    try:
        if request.method == 'POST':
            ...
            url_pokeapi = urllib.request.Request(f"https://pokeapi.co/api/v2/pokemon/{pokemon.lower()}")
            ...
            types = ', '.join([t['type']['name'].capitalize() for t in list_of_data['types']])
            abilities = ', '.join([a['ability']['name'].replace('-', ' ').capitalize() for a in list_of_data['abilities']])
            moves = ', '.join([m['move']['name'].replace('-', ' ').capitalize() for m in list_of_data['moves'][:5]])
            stats = ', '.join([f"{s['stat']['name'].capitalize()}: {s['base_stat']}" for s in list_of_data['stats']])

            url_species = urllib.request.Request(f'https://pokeapi.co/api/v2/pokemon-species/{pokemon.lower()}')
            ...
            data = {
                ...
                "imagen": list_of_data['sprites']['other']['official-artwork']['front_default'],
                "tipos": types,
